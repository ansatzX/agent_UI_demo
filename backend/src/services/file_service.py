...
class FileService:
    ...
    def generate_unique_filename(self, original_filename: str) -> str:
        ...
        # 清理原始文件名，移除特殊字符
        clean_name = re.sub(
            r"[^\w\u4e00-\u9fff]", "_", Path(original_filename).stem
        )
    ...
    async def process_uploaded_file(
        self, file_path: Path, unique_filename: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if ext == ".docx":
                ...
                result = await asyncio.to_thread(
                    self._parse_docx_simple, file_path
                )
            elif ext == ".doc":
            ...
            logger.info(
                f"文件解析完成: 段落={len(result.get('paragraphs', []))}, "
    ...
    def list_uploaded_files(self) -> list:
        ...
        for file_path in self.uploads_dir.glob("*.docx"):
            files.append(
                {"filename": file_path.name, "size": file_path.stat().st_size}
            )
